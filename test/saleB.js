...
contract('Sale', (accounts) => {
  ...
  describe('Sale period 0', () => {
    ...
    const endBlockError = 'was able to purchase after the endBlock';
    ...
    it('should fail to transfer 100 tokens to Edwhale.', async () => {
      ...
      try {
        ...
        const errMsg = edwhale + endBlockError;
        assert(false, errMsg);
      } catch (err) {
        const errMsg = err.toString();
        assert(isEVMException(err), errMsg);
        // Make sure balance is unchanged
        const finalBalance = await getTokenBalanceOf(edwhale);
        const errMsg2 = balanceError;
        assert.strictEqual(finalBalance.toString(10), startingBalance.toString(10), errMsg2);
      }
    });
  });
  ...
  describe('Post-sale period', () => {
    ...
    it('should report the proper sum of Wei in the wallet.', async () => {
      ...
      // The number of tokens successfully sold to James and Miguel
      const tokensSold = new BN('11', 10);
      const expected = tokensSold.mul(saleConf.price);
    ...
    it('should allow Miguel to transfer 10 tokens to James.', async () => {
      ...
      const miguelStartingBalance = await getTokenBalanceOf(miguel);
      const jamesStartingBalance = await getTokenBalanceOf(james);
      const sale = await Sale.deployed();
      const tokenAddr = await sale.token.call();
      const token = HumanStandardToken.at(tokenAddr);
      await as(miguel, token.transfer, james, transferAmount);
      const miguelFinalBalance = await getTokenBalanceOf(miguel);
      const miguelExpected = miguelStartingBalance.sub(transferAmount);
      const errMsg = balanceError;
      assert.strictEqual(
        miguelFinalBalance.toString(10), miguelExpected.toString(10), errMsg,
      );
  ...
  describe('Filters and disbursers', () => {
    ...
    // This simulation should run after the first (sale.js), so the time should
    // already be rolled up to the vesting period.
